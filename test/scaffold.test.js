...
test('scaffold: 骨架结构完整', () => {
  ...
  const res = scaffoldPack({ targetDir: ROOT, packId, versions: ['1.7.10', '26.3'], description: '测试包' })
  ...
  assert.deepEqual(cfg.versions, ['1.7.10', '26.3'])
...
test('scaffold: 默认全版本 + 校验参数', () => {
  ...
  assert.ok(res.versions.includes('26.3'))
  assert.ok(res.versions.includes('26.2'))
