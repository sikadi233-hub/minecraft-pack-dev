...
- 校验：`mc_pack_validate <本目录> -targetMc 26.2`（或 analyze / log 模式）
- 构建：`mc_pack_build <本目录>` → `dist/<版本>/{{packId}}.zip`
- 时代核对：pack_format 矩阵见 `minecraft-pack-core` 技能 references/pack-format-matrix.md

## 示例文件说明

- `source/cit/example_modern.properties` — 1.20.5+ / 26.2 组件时代条件（components.minecraft\:custom_name）
...
3. 目录/文件名不要含空格
4. 26.2 时代不要用 `nbt.display.*` 条件（lore 匹配已死）
