...
import { resolveEra, citRuntimeMissingFor } from './pack-format.js'
import {
...
export function validatePack(opts) {
    ...
    for (const e of p.entries) {
      ...
      if (key.startsWith('nbt.display.')) {
        if (era && era.components) {
          add('error', 'cit', f.rel, 'lore-dead', `nbt.display.* 条件在 ${targetMc}（组件时代）永不匹配（lore 已变组件 compound tags）——改用 components.minecraft\\:custom_name=ipattern:*…* 或 item_name`, `第 ${line} 行: ${key}=${value}`)
        } else if (!era) {
          add('warn', 'cit', f.rel, 'lore-era', 'nbt.display.* 条件在 1.20.5+ 组件时代失效（citresewn lore 匹配已死）——未指定 targetMc，无法核对', `第 ${line} 行: ${key}=${value}`)
        }
      }
      if (key === 'damage') {
  ...
  // ── CIT 前端可用性（非阻塞提示）──────────────────────────
  // 某些版本（截至 2026-09：26.3）资源包本体可用，但 CIT 前端模组
  // （cit-resewn-continuation / cit-resewn-fork）尚无对应版本 → 包内
  // .properties 没有运行时可执行。这**不是**包的错误，只提示用户。
  if (stats.citProperties > 0 && targetMc && citRuntimeMissingFor(targetMc)) {
    add(
      'warn', 'cit', 'cit/', 'cit-runtime-missing',
      `${targetMc} 上没有可用的 CIT 前端（cit-resewn-continuation / cit-resewn-fork 截至 2026-09 无该版本）——包内 ${stats.citProperties} 个 .properties 文件不会被任何运行时加载；资源包其余内容不受影响`,
    )
  }
